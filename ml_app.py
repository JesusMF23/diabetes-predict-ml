...
def run_ml_app():
	...
	with st.expander("Resultados de la Predicción"):
		...
		if prediction == 1:
			...
			st.subheader("Valores de Predicción en probabilidades")
			st.json(pred_probability_score)
########## Esta sección de código no se puede probar porque el modelo anterior está entrenado con una versión de scikit-learn demasiado vieja e incompatible.
##### No puedo downgradear versiones porque me da problemas con otros paquetes y me obliga a bajar versiones de Python.
 	# # # Extra2: Predicción usando el modelo adicional (Árbol de Decisión)
	# # with st.expander("Resultados de la Predicción - Modelo Árbol de Decisión (Extra)"):
	# # 	try:
	# # 		decision_tree_model = load_model("models/decision_tree_model_diabetes_21_oct_2020.pkl")
	# # 		single_sample = np.array(encoded_result).reshape(1, -1)
	# # 		dt_prediction = decision_tree_model.predict(single_sample)
	# # 		dt_pred_prob = decision_tree_model.predict_proba(single_sample)
	# # 		st.write("Predicción (Árbol de Decisión):", dt_prediction)
	# # 		if dt_prediction == 1:
	# # 			st.warning("Riesgo Positivo - {}".format(dt_prediction[0]))
	# # 		else:
	# # 			st.success("Riesgo Negativo - {}".format(dt_prediction[0]))
	# # 		dt_pred_probability_score = {
	# # 			"Negativo en DM": dt_pred_prob[0][0]*100,
	# # 			"Positivo en DM": dt_pred_prob[0][1]*100
	# # 		}
	# # 		st.subheader("Valores de Predicción en probabilidades (Árbol de Decisión)")
	# # 		st.json(dt_pred_probability_score)
	# # 	except Exception as e:
	# # 		st.error("Error during Decision Tree prediction: " + str(e))
